...
self.onmessage = async (e: MessageEvent<WorkerMessage>) => {
  ...
  switch (data.type) {
    case "init-analyzer":
      ...
      // Initialize Wasm using the shared memory
      ...
      const initMeta = new DataView(wasmMemory.buffer, MEMORY_MAP.METADATA_OFFSET, MEMORY_MAP.METADATA_SIZE);
    ...
    case "analyze":
      ...
      const wasmInput = new Float32Array(wasmMemory.buffer, MEMORY_MAP.FFT_INPUT_OFFSET, fftSize);
      wasmInput.set(inBuf.subarray(index, index + fftSize));

      // Update current time in metadata
      const metaView = new DataView(wasmMemory.buffer, MEMORY_MAP.METADATA_OFFSET, MEMORY_MAP.METADATA_SIZE);
